...
// private:
	...
	void finalize()
	{
		...
		for(size_t i = 0; i < dest_permute.size(); i++)
		{
			...
			csr_values.push_back(std::pair<local_edge_buffer.target_arr[pos],
											pos>)
		}
		for(size_t i = 0; i < src_permute.size(); i++)
		{
			auto pos = src_permute[i];
			csc_values.push_back(std::pair<local_edge_buffer.source_arr[pos],
											pos>)
		}
	...
	out_edges(lvid_type v)
	{
		...
		return std::vector<std::pair<lvid_type, edge_id_type>> temp(
